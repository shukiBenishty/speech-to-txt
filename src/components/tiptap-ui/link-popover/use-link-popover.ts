...
export function shouldShowLinkButton(props: {
  ...
  const { editor, hideWhenUnavailable, canSetLink: canSetLinkState } = props
...
export function useLinkState(props: {
  ...
  const { canSet= false, isActive= false } = useEditorState({
    editor,
    selector: (state) => {
      ...
      return {
        canSet: !isCode || (isCode && isSame) ? canSetLink(editor) : false,
