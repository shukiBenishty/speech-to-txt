...
export function shouldShowButton(props: {
  ...
  const { editor, type, hideWhenUnavailable, canToggle: canToggleState } = props
...
export function useList(config: UseListConfig) {
  ...
  const { isActive= false, canToggle= false } = useEditorState({
    editor,
    selector: (state) => {
      ...
      return {
        ...
        canToggle:
          !isCode || (isCode && isSame)
            ? canToggleList(editor, type, false)
            : false,
      }
    },
  }) || {}
