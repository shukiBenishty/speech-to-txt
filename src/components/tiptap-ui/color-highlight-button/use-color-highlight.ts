...
export function shouldShowButton(props: {
  ...
  const { editor, hideWhenUnavailable, canColorHighlight: canHighlight } = props
...
export function useColorHighlight(config: UseColorHighlightConfig) {
  ...
  const { isActive= false, canColorHighlight: canColorHighlightState= false } = useEditorState({
    editor,
    selector: (state) => {
      ...
      return {
        isActive: isColorHighlightActive(editor, highlightColor),
        canColorHighlight:
          !isCode || (isCode && isSame) ? canColorHighlight(editor) : false,
      }
    },
  }) || {}
